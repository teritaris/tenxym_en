...
class Listener {
    ...
    newBlock() {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.share(), operators_1.filter((_) => _.channelName === ListenerChannelName.block), operators_1.map((_) => _.message));
    }
    /**
    ...
    finalizedBlock() {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.share(), operators_1.filter((_) => _.channelName === ListenerChannelName.finalizedBlock), operators_1.map((_) => _.message));
    }
    /**
    ...
    transactionSubscription(channel, unresolvedAddress, transactionHash, subscribeMultisig = false) {
        return this.subscribeWithMultig(unresolvedAddress, channel, subscribeMultisig).pipe(operators_1.switchMap((subscribers) => {
            return this.messageSubject.asObservable().pipe(operators_1.filter((listenerMessage) => listenerMessage.channelName === channel), operators_1.distinctUntilChanged((prev, curr) => {
    ...
    status(unresolvedAddress, transactionHash) {
        return this.getResolvedAddress(unresolvedAddress).pipe(operators_1.mergeMap((address) => {
            ...
            return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName === ListenerChannelName.status), operators_1.filter((_) => _.channelParam.toUpperCase() === address.plain()), operators_1.map((_) => _.message), operators_1.filter((_) => !transactionHash || _.hash.toUpperCase() == transactionHash.toUpperCase()));
        }));
    }
    /**
    ...
    cosignatureAdded(unresolvedAddress, subscribeMultisig = false) {
        return this.subscribeWithMultig(unresolvedAddress, ListenerChannelName.cosignature, subscribeMultisig).pipe(operators_1.switchMap((subscribers) => {
            return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName.toUpperCase() === ListenerChannelName.cosignature.toUpperCase()), operators_1.filter((_) => subscribers.includes(_.channelParam.toUpperCase())), operators_1.map((_) => _.message));
        }));
    }
    /**
     * @internal
    ...
    subscribeWithMultig(cosigner, channel, multisig = false) {
        ...
        return this.getResolvedAddress(cosigner).pipe(operators_1.mergeMap((address) => {
            return this.multisigRepository.getMultisigAccountGraphInfo(address).pipe(operators_1.map((multisigInfo) => {
                ...
                if (!!multisigChildren.length && multisigChildren[0].children) {
                    Utilities_1.parseObjectProperties(multisigChildren[0].children, (k, prop) => {
