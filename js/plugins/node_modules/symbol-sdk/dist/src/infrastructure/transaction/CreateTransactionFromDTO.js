 ...
 * @constructor
 */
const CreateStandaloneTransactionFromDTO = (transactionDTO, transactionInfo, isEmbedded) => {
    ...
    switch (type) {
        case transaction_1.TransactionType.TRANSFER:
            return new transaction_1.TransferTransaction(networkType, version, deadline, maxFee, exports.extractRecipient(transactionDTO.recipientAddress), exports.extractMosaics(transactionDTO.mosaics), model_1.MessageFactory.createMessageFromHex(transactionDTO.message), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        ...
        case transaction_1.TransactionType.MOSAIC_SUPPLY_REVOCATION:
            return new model_1.MosaicSupplyRevocationTransaction(networkType, version, deadline, maxFee, exports.extractRecipient(transactionDTO.sourceAddress), new mosaic_1.Mosaic(new mosaic_1.MosaicId(transactionDTO.mosaicId), model_1.UInt64.fromNumericString(transactionDTO.amount)), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.MULTISIG_ACCOUNT_MODIFICATION:
            return new transaction_1.MultisigAccountModificationTransaction(networkType, version, deadline, maxFee, transactionDTO.minApprovalDelta, transactionDTO.minRemovalDelta, transactionDTO.addressAdditions ? transactionDTO.addressAdditions.map((addition) => exports.extractRecipient(addition)) : [], transactionDTO.addressDeletions ? transactionDTO.addressDeletions.map((deletion) => exports.extractRecipient(deletion)) : [], signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        ...
        case transaction_1.TransactionType.SECRET_LOCK:
            ...
            return new transaction_1.SecretLockTransaction(networkType, version, deadline, maxFee, new mosaic_1.Mosaic(mosaicId, model_1.UInt64.fromNumericString(transactionDTO.amount)), model_1.UInt64.fromNumericString(transactionDTO.duration), transactionDTO.hashAlgorithm, transactionDTO.secret, exports.extractRecipient(recipientAddress), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.SECRET_PROOF:
            return new transaction_1.SecretProofTransaction(networkType, version, deadline, maxFee, transactionDTO.hashAlgorithm, transactionDTO.secret, exports.extractRecipient(transactionDTO.recipientAddress), transactionDTO.proof, signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        ...
        case transaction_1.TransactionType.ADDRESS_ALIAS:
            return new transaction_1.AddressAliasTransaction(networkType, version, deadline, maxFee, transactionDTO.aliasAction, namespace_1.NamespaceId.createFromEncoded(transactionDTO.namespaceId), exports.extractRecipient(transactionDTO.address), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.ACCOUNT_ADDRESS_RESTRICTION:
            return new transaction_1.AccountAddressRestrictionTransaction(networkType, version, deadline, maxFee, transactionDTO.restrictionFlags, transactionDTO.restrictionAdditions
                ? transactionDTO.restrictionAdditions.map((addition) => exports.extractRecipient(addition))
                : [], transactionDTO.restrictionDeletions
                ? transactionDTO.restrictionDeletions.map((deletion) => exports.extractRecipient(deletion))
                : [], signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        ...
        case transaction_1.TransactionType.MOSAIC_ADDRESS_RESTRICTION:
            return new transaction_1.MosaicAddressRestrictionTransaction(networkType, version, deadline, maxFee, utils_1.UnresolvedMapping.toUnresolvedMosaic(transactionDTO.mosaicId), model_1.UInt64.fromHex(transactionDTO.restrictionKey), exports.extractRecipient(transactionDTO.targetAddress), model_1.UInt64.fromNumericString(transactionDTO.previousRestrictionValue), model_1.UInt64.fromNumericString(transactionDTO.newRestrictionValue), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.ACCOUNT_METADATA:
            return new transaction_1.AccountMetadataTransaction(networkType, version, deadline, maxFee, exports.extractRecipient(transactionDTO.targetAddress), model_1.UInt64.fromHex(transactionDTO.scopedMetadataKey), transactionDTO.valueSizeDelta, format_1.Convert.decodeHex(transactionDTO.value), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.MOSAIC_METADATA:
            return new transaction_1.MosaicMetadataTransaction(networkType, version, deadline, maxFee, exports.extractRecipient(transactionDTO.targetAddress), model_1.UInt64.fromHex(transactionDTO.scopedMetadataKey), utils_1.UnresolvedMapping.toUnresolvedMosaic(transactionDTO.targetMosaicId), transactionDTO.valueSizeDelta, format_1.Convert.decodeHex(transactionDTO.value), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
        case transaction_1.TransactionType.NAMESPACE_METADATA:
            return new transaction_1.NamespaceMetadataTransaction(networkType, version, deadline, maxFee, exports.extractRecipient(transactionDTO.targetAddress), model_1.UInt64.fromHex(transactionDTO.scopedMetadataKey), namespace_1.NamespaceId.createFromEncoded(transactionDTO.targetNamespaceId), transactionDTO.valueSizeDelta, format_1.Convert.decodeHex(transactionDTO.value), signature, signer, transactionInfo).setPayloadSize(transactionDTO.size);
...
exports.CreateTransactionFromDTO = (transactionDTO) => {
    if (transactionDTO.transaction.type === transaction_1.TransactionType.AGGREGATE_COMPLETE ||
        ...
        return CreateStandaloneTransactionFromDTO(transactionDTO.transaction, extractTransactionMeta(transactionDTO.meta, transactionDTO.id), false);
    }
};
